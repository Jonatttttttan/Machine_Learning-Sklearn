...
def computeMultipleLinearRegressionModel(XTrain, XTest, yTrain, yTest):
...
def runMultipleLinearRefressionExample(filename):
    ...
    elapsed_time = time.time() - start_time
    print("Split Dataset: %.2f" % elapsed_time, "segundos")

    start_time = time.time()
    computeMultipleLinearRegressionModel(XTrain, XTest, yTrain, yTest)
    elapsed_time = time.time() - start_time
    print("Compute Multiple Linear Regression: %.2f" % elapsed_time, "segundos")

if __name__ == "__main__":
    runMultipleLinearRefressionExample(caminho)
