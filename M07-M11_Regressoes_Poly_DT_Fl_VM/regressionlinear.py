...
def showPlot(X, y, linear_regressor):
...
def runLinearRegressionExamplpe(filename):
    start_time = time.time()
    X, y, csv = pre.loadDataset(filename)
    elapsed_time = time.time() - start_time

    start_time = time.time()
    regressor = computeLinearRegression(X, y)
    elapsed_time = time.time() - start_time
...
if __name__ == "__main__":
    print(runLinearRegressionExamplpe())
